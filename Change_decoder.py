...
def create_data():
	...
	train_mask = np.isin(Y_train, [0,5,9])
	test_mask = np.isin(Y_test, [0,5,9])
	...
	y_train[y_train == 5] = y_train [y_train==5] -4
	y_test[y_test==5] = y_test [y_test==5] -4
	
	y_train[y_train == 9] = y_train [y_train==9] -7
	y_test[y_test==9] = y_test [y_test==9] -7
	
	y_train[y_train == 0] = y_train [y_train==0] 
	y_test[y_test==0] = y_test [y_test==0] 
...
def L2X(datatype="mnist", train = True):
	...
	preds = Dense(3, activation='softmax', name = 'dense4',
		kernel_regularizer=regularizers.l2(1e-3))(net)
	...
	model = Model(inputs=model_input, outputs=[preds, new_model_input3])
	model.summary()
	...
	pred_model = Model(model_input, [samples, samples_grp,preds])
	...
	scores, scores_grp,preds = pred_model.predict(x_val, verbose = 1, batch_size = BATCH_SIZE)
	...
	return median_ranks, time.time() - st2, st2 - st1, scores, scores_grp,x_val,y_val,preds
...
if __name__ == '__main__':
	...
	median_ranks, exp_time, train_time,scores, scores_grp,x_val,y_val,preds = L2X(train = args.train)
	...
	print (np.shape(x_val),"123123213")
