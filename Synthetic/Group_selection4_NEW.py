...
def L2X(datatype, train = True):
	...
	if train:
		...
		model.fit(x_train, [y_train, x_train], validation_data=(x_val, [y_val, x_val]),callbacks = callbacks_list, epochs=4, batch_size=BATCH_SIZE)
		st2 = time.time()
	else:
	...
	return median_ranks, time.time() - st2, st2 - st1, scores, scores_grp,x_val,y_val,datatype_val
...
if __name__ == '__main__':
	...
	median_ranks, exp_time, train_time,scores, scores_grp, x_val, y_val,datatype_val = L2X(datatype = args.datatype,
		train = args.train)
