...
def L2X(datatype, train = True):
	...
	# here we apply instance-wise feature selection again
	...
	new_model_input = Multiply()([new_model_input, samples_grp]) 
	...
	if train:
		...
		model.fit(x_train, y_train, validation_data=(x_val, y_val),callbacks = callbacks_list, epochs=2, batch_size=BATCH_SIZE)
		st2 = time.time()
	else:
...
if __name__ == '__main__':
	...
	median_ranks, exp_time, train_time,scores, scores_grp, x_val, y_val = L2X(datatype = args.datatype,
		train = args.train)
